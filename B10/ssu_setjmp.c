...
int main(void)
{
	register int loc_register; // register변수
	volatile int loc_volatile; // volatile 변수
	int loc_var; // 지역 변수
	...
	if (setjmp(glob_buffer) != 0) { // setjump세팅
	...
	loc_volatile = 81;
	loc_register = 83;
	ssu_nested_func(loc_var, loc_volatile, loc_register); // longjmp를 호출
	exit(0);
}
