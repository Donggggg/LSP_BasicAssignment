...
int main(void)
{
	if (signal(SIGUSR1, ssu_func) == SIG_ERR) { // SIGUSR1 시그널 시 ssu_func
		fprintf(stderr, "SIGUSR1 error");
		exit(1);
	}

	if (signal(SIGALRM, ssu_alarm) == SIG_ERR) { // SIGALRM 시그널 시 ssu_alarm
...
static void ssu_func(int signo) { 
	...
	while (1)
		if (time(NULL) > start_time + 5) // 5초 후 break
			break;

	ssu_mask("ending ssu_func: ");
	can_jump = 0;
	siglongjmp(jump_buf, 1); // jump..
}
